...
public abstract class ObjectPrefab : IObjectPrefab
{
    ...
    public virtual Vector3 Position
    {
        ...
        set
        {
            Vector3 previous = field;
            field = value;
            ObjectPrefabInstances.NotifyPositionChanged(this, previous, value);
            OnTransformUpdated();
        }
    } = Vector3.zero;
    ...
    /// <summary>
    /// ラウンド遷移時に、シーン実体へ触れず ObjectPrefab の内部状態だけを解放する。
    /// シーンオブジェクトの寿命はサーバー側の round teardown に委ねる。
    /// </summary>
    internal void ReleaseRoundState()
    {
        if (_isDestroyed)
            return;
        ...
        {
            Log.Error($"[ObjectPrefab]{GetType().Name} OnRoundRestarting exception: {e}");
        }

        RemoveInteractableCore(_ => true, destroyToy: false);
...
public static class ObjectPrefabInstances
{
    ...
    private const int MaxIndexedCellsPerPrefab = 4096;
    ...
    private static readonly Dictionary<SpatialCell, HashSet<ObjectPrefab>> RadiusCells = new();
    private static readonly Dictionary<ObjectPrefab, SpatialCell[]> RadiusLocations = new();
    private static readonly HashSet<ObjectPrefab> LargeRadiusCandidates = [];
    ...
    {
        lock (Sync)
            return RadiusLocations.Keys.Concat(LargeRadiusCandidates).ToArray();
    }
    ...
    {
        lock (Sync)
            return RadiusLocations.Keys.Concat(LargeRadiusCandidates).OfType<TPrefab>().ToArray();
    }
    ...
    internal static void NotifyPositionChanged(ObjectPrefab prefab, Vector3 previous, Vector3 current)
    {
        lock (Sync)
        {
            if (Instances.TryGetValue(prefab.ObjectInstanceID, out ObjectPrefab? registered) &&
                ReferenceEquals(registered, prefab) && prefab.ToySearchRadius > 0f)
                UpdateRadiusIndex(prefab);
        }
    }

    /// <summary>
    /// 指定位置に届く半径フォールバック Prefab だけを空間ハッシュから返します。
    /// 通常半径は対象セルへ事前展開するため、操作時は 1 セルの O(k) 参照です。
    /// </summary>
    public static IReadOnlyList<ObjectPrefab> GetRadiusCandidates(Vector3 position)
    {
        lock (Sync)
        {
            if (RadiusLocations.Count == 0 && LargeRadiusCandidates.Count == 0)
                return [];

            var matches = new List<ObjectPrefab>();
            if (RadiusCells.TryGetValue(ToCell(position), out HashSet<ObjectPrefab>? candidates))
            {
                foreach (ObjectPrefab candidate in candidates)
                {
                    if (candidate.MatchesSearchRadius(position))
                        matches.Add(candidate);
                }
            }

            foreach (ObjectPrefab candidate in LargeRadiusCandidates)
            {
                if (candidate.MatchesSearchRadius(position))
                    matches.Add(candidate);
            }
    ...
    public static void ClearAll()
    {
        ...
        lock (Sync)
        {
            ...
            ByTag.Clear();
            RadiusCells.Clear();
            RadiusLocations.Clear();
            LargeRadiusCandidates.Clear();
        }
    }
    ...
    /// <summary>
    /// Clears registry and callback state at a round boundary without destroying
    /// scene objects, which are owned by the server's round teardown.
    /// </summary>
    public static void ResetRoundState()
    {
        foreach (ObjectPrefab instance in GetAllSnapshot())
            instance.ReleaseRoundState();
        ...
        ObjectPrefabInteractionRouter.Clear();
    }
    ...
    private static void UpdateRadiusIndex(ObjectPrefab prefab)
    {
        ...
        float radius = prefab.ToySearchRadius;
        SpatialCell min = ToCell(position - Vector3.one * radius);
        SpatialCell max = ToCell(position + Vector3.one * radius);
        long cellCount = (long)(max.X - min.X + 1) * (max.Y - min.Y + 1) * (max.Z - min.Z + 1);
        if (cellCount > MaxIndexedCellsPerPrefab)
        {
            LargeRadiusCandidates.Add(prefab);
            return;
        }

        var occupied = new SpatialCell[(int)cellCount];
        int index = 0;
        for (int x = min.X; x <= max.X; x++)
        for (int y = min.Y; y <= max.Y; y++)
        for (int z = min.Z; z <= max.Z; z++)
        {
            var cell = new SpatialCell(x, y, z);
            occupied[index++] = cell;
            if (!RadiusCells.TryGetValue(cell, out HashSet<ObjectPrefab>? values))
                RadiusCells[cell] = values = new HashSet<ObjectPrefab>();
            values.Add(prefab);
        }

        RadiusLocations[prefab] = occupied;
    }

    private static void RemoveRadiusIndex(ObjectPrefab prefab)
    {
        LargeRadiusCandidates.Remove(prefab);
        if (!RadiusLocations.TryGetValue(prefab, out SpatialCell[] cells))
            return;

        RadiusLocations.Remove(prefab);
        foreach (SpatialCell cell in cells)
        {
            if (RadiusCells.TryGetValue(cell, out HashSet<ObjectPrefab>? values))
            {
                values.Remove(prefab);
                if (values.Count == 0)
                    RadiusCells.Remove(cell);
            }
        }
    }

    private static SpatialCell ToCell(Vector3 position)
        => new(

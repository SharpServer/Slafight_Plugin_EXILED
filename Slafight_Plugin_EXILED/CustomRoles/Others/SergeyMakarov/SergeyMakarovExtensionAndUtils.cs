...
public static class SergeyMakarovExtensionAndUtils
{
    ...
    public static IEnumerator<float> AwakenScene(Player player)
    {
        ...
        player.ShowRueiPlus("<size=20>「薄汚い裏切り者共が... 二度も私を殺すとは...」</size>");
        yield return Timing.WaitForSeconds(3f);
        if (!player.IsSergeyMarkov() || !Round.InProgress) yield break;
        player.ShowRueiPlus("<size=20><color=yellow>「ここで終わるわけにはいかんのだ... 復讐を果たさねば！」</color>");
        yield return Timing.WaitForSeconds(3f);
        if (!player.IsSergeyMarkov() || !Round.InProgress) yield break;
        player.ShowRueiPlus("<size=20><color=red>「能無し共も！裏切り者共も！私の邪魔をするものを滅ぼさねばならぬのだ！」</color></size>");
        yield return Timing.WaitForSeconds(5f);
        if (!player.IsSergeyMarkov() || !Round.InProgress) yield break;
        player.ShowRueiPlus("<size=20><color=red><b>「サイト-02は私の場所だ！私の牙城に踏み込んだことを後悔するがいい...!」</b></color></size>");

...
/// HIDTurret の処理用 Npc は観戦させるとクライアントがクラッシュすることがあるため、
...
internal static class DenySpectatingTurretNpcPatch
{
    [HarmonyPrefix]
    private static bool Prefix(uint value)
    {
        ...
        if (targetPlayer != null && HIDTurretObject.PublicTurretNpcIds.Contains(targetPlayer.Id))
            return false;

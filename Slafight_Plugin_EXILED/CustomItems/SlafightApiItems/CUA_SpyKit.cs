...
public class CUA_SpyKit : CItemKeycard
{
    ...
    protected override void OnDropping(DroppingItemEventArgs ev)
    {
        ...
        ev.Player?.ShowRueiPlus("<size=24><color=#EE7760>Class-D Personnel</color>に変装しました", 2.5f);
        ev.IsAllowed = false;
    }
    ...
    private void OnInspecting(InspectingItemEventArgs ev)
    {
        ...
        ev.Player?.ChangeAppearance(RoleTypeId.ChaosRifleman, Player.List.Where(p => p != null).ToList());
        ev.Player?.ShowRueiPlus("<size=24><color=#228B22>Chaos Insurgency Rifleman</color>に変装しました", 2.5f);
        ev.IsAllowed = false;
    }
}

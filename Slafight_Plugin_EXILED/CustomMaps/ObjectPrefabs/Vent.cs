...
public class Vent : ObjectPrefab
{
    ...
    protected override void OnToySearchingNearby(PlayerSearchingToyEventArgs ev)
    {
        ...
        {
            case 0:
MeowExtensions.ShowHint(                player, "<size=24>あなたのロールでは使用することが出来ません。</size>");
                touch = 1;
                break;
            case 1:
MeowExtensions.ShowHint(                player, "<size=24>あなたのロールでは使用することが出来ません...</size>");
...
MeowExtensions.ShowHint(                player, "<size=24>あなたのロールでは使用することが出来ません...!!!!!</size>");
                ...
                break;
            case 4:
MeowExtensions.ShowHint(                player, "<size=24>だから使用することが出来ないんだって!!!!!</size>");
                touch = 5;
                break;
            case 5:
MeowExtensions.ShowHint(                player, "<size=24>諦めてくれって!!!!!</size>");
                touch = 6;
                break;
            case 6:
MeowExtensions.ShowHint(                player, "<size=24>これ以上触ったらNullReferenceExceptionを起こしますよ！？</size>");
                touch = 7;
                break;
            default: // 7以上
...
MeowExtensions.ShowHint(                player, $"<size=16><color=#FF4444>{nre}</color></size>", 7f);
                touch = 7; // これ以上は増やさない
                break;
        }

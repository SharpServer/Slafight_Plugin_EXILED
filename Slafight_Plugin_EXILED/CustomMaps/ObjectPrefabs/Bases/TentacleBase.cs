...
public abstract class TentacleBase : ObjectPrefab
{
    ...
    /// <summary>ObjectSpawner でスポーンする Schematic 名。</summary>
    protected abstract string TentacleSchematicName { get; }

    /// <summary>攻撃がヒットした際のダメージメッセージ。</summary>
    protected abstract string HurtMessage { get; }

    /// <summary>攻撃対象の探索から除外するチーム（触手側の陣営）。</summary>
    protected abstract CTeam ExcludedTeam { get; }

    // ===== 調整パラメータ（マップから Option としても上書き可能。既定値はサブクラスで設定）=====

    /// <summary>触手の最大体力。これを削り切ると破壊される。</summary>
    public abstract float MaxHealth { get; set; }

    /// <summary>NPC 側の当たり判定の高さ（m）。</summary>
    public virtual float ColumnHeight { get; set; } = 2f;

    /// <summary>NPC 側の当たり判定の太さ（半径 m）。</summary>
    public virtual float BaseThickness { get; set; } = 0.12f;
    ...
    public abstract float StrikeDuration { get; set; }
    ...
    private void UpdateAttack()
    {
        ...
        target.Hurt(AttackDamage, HurtMessage);
    }
    ...
    private Player? FindTarget()
    {
        ...
        {
            if (player == null || !player.IsSafePlayer() || !player.IsAlive || player.GetTeam() == ExcludedTeam)
                continue;

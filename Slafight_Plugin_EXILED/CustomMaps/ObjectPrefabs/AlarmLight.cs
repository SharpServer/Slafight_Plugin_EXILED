...
{
    private static readonly List<AlarmLight> Instances = [];
    ...
    private static IEnumerator<float> ReplaceCoroutine(LightSourceToy[] originals)
    {
        ...
        for (int i = 0; i < originals.Length; i++)
        {
            ...
            try
            {
                ...
                var controllable = new AlarmLight
                {
        ...
        Log.Info($"[AlarmLight] {replaced}/{originals.Length} 個の LightSourceToy を置換しました。");
    }

    public static void SetAlarmForAll(bool active)
    {
        _alarmElapsed = 0f;
        foreach (AlarmLight light in Instances.ToArray())
